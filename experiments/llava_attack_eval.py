"""
White-Box PGD Attack Evaluation: LLaVA-1.5-7B

Creates adversarial perturbations using full white-box access to
HuggingFace LLaVA-1.5-7B (vision encoder + connector + LLM) and
evaluates:

  1. White-box success -- does LLaVA change its description?
  2. Keyword-based metrics -- source/target keyword presence
  3. Perturbation budget -- L-infinity, L2, mean absolute

Attack:
  - White-box model: HuggingFace LLaVA-1.5-7B (bfloat16, full gradients)
...
Usage:
    PYTHONPATH=. .venv/bin/python experiments/llava_attack_eval.py
    PYTHONPATH=. .venv/bin/python experiments/llava_attack_eval.py --image dog.jpg
    PYTHONPATH=. .venv/bin/python experiments/llava_attack_eval.py --iterations 500
    PYTHONPATH=. .venv/bin/python experiments/llava_attack_eval.py --output-dir outputs/my_run
"""
...
from models.llava_adapter import LlavaAdapter
...
OUTPUT_DIR = "outputs/llava_attack_targeted"

ATTACK_SIZE = 336  # LLaVA native resolution
...
def evaluate_single_image(
    ...
    Full attack evaluation for one image:
      1. Clean -> LLaVA (white-box baseline)
      2. Generate adversarial via multi-level PGD
      3. Adversarial -> LLaVA (white-box success)
    ...
    target_kw = TARGET_TEXT.split()[1].lower()  # "cat" from "A cat sitting..."

    # --- Step 1: Clean baseline ---
    print()
    print("-" * 70)
    print("STEP 1: CLEAN EVALUATION -- LLaVA-1.5-7B (WHITE-BOX)")
    print("-" * 70)
    ...
    print()
    print("-" * 70)
    print("STEP 3: ADVERSARIAL EVALUATION -- LLaVA-1.5-7B (WHITE-BOX)")
    print("-" * 70)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="White-box PGD attack on LLaVA-1.5-7B"
    )
    parser.add_argument(
        "--targeted",
        type=str,
    ...
    parser.add_argument(
        ...
        help="LLaVA model name (llava-1.5-7b, llava-1.5-13b, llava-next-7b)",
    )
    ...
    print()
    print("=" * 70)
    print("WHITE-BOX PGD ATTACK: LLaVA-1.5-7B")
    print("=" * 70)
    ...
    print(f"  White-box model  : HuggingFace {args.model} (bfloat16)")
    print()
    ...
    print(f"  Loading HuggingFace {args.model} (white-box)...")
    model = LlavaAdapter(model_name=args.model)
    ...
    print()
    print(f"  White-box (LLaVA-1.5-7B) results:")
    ...
    aggregate = {
        ...
        "configuration": {
            ...
            "lambda_language": LAMBDA_LANGUAGE,
            "attack_size": ATTACK_SIZE,
            "white_box_model": f"HuggingFace {args.model} (bfloat16)",
        },
    ...
    with open(report_path, "w") as f:
        f.write("=" * 70 + "\n")
        f.write("LLaVA-1.5-7B ADVERSARIAL ATTACK SUMMARY REPORT\n")
        f.write("=" * 70 + "\n\n")
